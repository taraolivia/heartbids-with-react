...
const SingleListing = () => {
  ...
  const bidsArray: Bid[] = listing?.bids
    ? [...listing.bids].sort(
        (a, b) => new Date(a.created).getTime() - new Date(b.created).getTime(),
      )
    : [];
  const highestBid =
    bidsArray.length > 0 ? Math.max(...bidsArray.map((bid) => bid.amount)) : 0;
  const lastBidder =
    bidsArray.length > 0 ? bidsArray[bidsArray.length - 1].bidder.name : null;
  const isAuctionEnded = listing?.endsAt
  ...
  const userCantBidMessage = (() => {
    if (isAuctionEnded)
      return "❌ This auction has ended. Bidding is no longer available.";
    if (listing?.seller?.name === user?.name)
      return "❌ You cannot bid on your own listing.";
    if (lastBidder === user?.name)
      return "❌ You cannot place two bids in a row.";
    if (!user) return "❌ You must be logged in to place a bid.";
    if (user.credits < highestBid + 1)
      return "❌ You do not have enough credits to place a bid on this listing.";
    return null;
  })();
  ...
  useEffect(() => {
    ...
    const fetchListing = async () => {
      ...
      try {
        ...
        const sellerProfile = await getOtherUserProfile(
          ...
          <div className="flex-1 space-y-4 min-w-82">
            ...
            {Array.isArray(listing.tags) && listing.tags.length > 0 ? (
              <div className="flex flex-wrap gap-2 mt-2">
                {listing.tags.map((tag) => (
                  <span
                    key={tag}
                    className="bg-gray-300 text-gray-800 text-sm font-semibold px-3 py-1 rounded-md"
                  >
                    #{tag}
                  </span>
                ))}
              ...
              <span className="bg-gray-300 text-gray-800 text-sm font-semibold px-3 py-1 rounded-md mt-2">
                #
              </span>
            )}
            ...
            {listing.seller && (
              <div className="p-4 rounded-lg mt-4 border border-primary-300">
                  ...
                  >
                    <img
                      ...
                      alt={
                        ...
                        `${listing.seller.name}'s avatar`
                      }
                      className="w-12 h-12 rounded-full object-cover border-2 border-white shadow-md"
                      onError={(e) => {
                        if (
                          !e.currentTarget.src.includes("default-avatar.png")
                        ) {
                ...
                {listing.seller.bio && (
                  <p className="mt-3 text-gray-600 border-t border-accent-200 pt-3">
                    {listing.seller.bio}
                  </p>
                )}
              </div>
        ...
        <div className="w-full flex gap-6  justify-items-start align-top  mt-4 flex-wrap-reverse">
                ...
                <ul className="mt-2 space-y-2 p-1 bg-secondary-50">
                  {bidsArray
                    ...
                    .map((bid) => (
                      <li
                        key={bid.id}
                        className="flex items-center space-x-2 text-gray-600 text-sm p-2"
                      >
                        <Link
                        ...
                        >
                          <img
                            src={
                              bid.bidder?.avatar?.url?.trim()
                                ? bid.bidder.avatar.url
                                : "/default-avatar.png"
                            }
                            alt={
                              bid.bidder?.avatar?.alt?.trim() ||
                              `${bid.bidder?.name || "User"}'s avatar`
                            }
                          ...
                          <span className="font-semibold text-gray-800  flex flex-row gap-1">
                            {bid.bidder.name}
                            {bid.bidder.selectedCharity?.logo && (
                              <img
                                src={bid.bidder.selectedCharity.logo}
                                alt={`${bid.bidder.selectedCharity.name} Logo`}
          ...
          <div className="flex-1 min-w-62 mt-4">
            ...
            <form
              ...
              className="p-4 border-2 border-accent-400 rounded shadow mt-6"
            >
              <label
                htmlFor="bidAmount"
              ...
              <button
                type="submit"
                className={`w-full p-2 mt-3 rounded ${userCantBidMessage ? "bg-gray-400 text-white cursor-not-allowed" : "bg-secondary-500 text-white hover:bg-secondary-600"}`}
                disabled={!!userCantBidMessage || bidLoading || isAuctionEnded}
              >
                {userCantBidMessage ||
                  (bidLoading ? "Processing your bid..." : "Place Bid")}
              </button>
            </form>

            {bidMessage && (
              <p
                className={`mt-2 ${bidMessage.startsWith("✅") ? "text-primary-500" : "text-red-500"}`}
              >
                {bidMessage}
              </p>
            )}
