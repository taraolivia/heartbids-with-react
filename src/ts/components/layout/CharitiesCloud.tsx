...
const CharitiesCloud: React.FC = () => {
  return (
    <section className="py-16 font-serif bg-gradient-to-b from-secondary-400 via-accent-200 to-background-50 text-center">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
        ...
        <div className="flex flex-wrap justify-center gap-x-16 gap-y-15 mb-18">
          {AllCharities.map((charity, index) => (
            <div
              key={charity.name}
              className={`flex items-center space-x-3 ${index % 2 === 0 ? "translate-y-2" : "-translate-y-2"}`}
            >
              <img
                src={charity.logo}
                alt={charity.name}
      ...
      <Link
        to="/charities"
        className="p-3 bg-secondary-600 rounded-2xl text-white text-lg hover:bg-secondary-700"
      >
