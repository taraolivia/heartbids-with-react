...
const MostPopularListings: React.FC = () => {
  ...
  let filteredListings = listings.filter(
    (lot) => new Date(lot.endsAt).getTime() > Date.now(),
  );

  if (showOnlyHeartBids) {
    filteredListings = filteredListings.filter((lot) =>
      lot.tags?.includes("HeartBids"),
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {canScrollLeft && (
          <button
            ...
            className="text-2xl z-50 absolute left-2 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white px-7 py-4 cursor-pointer rounded-full shadow-md hover:bg-gray-700"
          >
            ❮
          </button>
        )}
        ...
        <div
          ref={scrollRef}
          className="flex overflow-x-auto gap-10 py-10 no-scrollbar scroll-smooth"
        >
        ...
        {canScrollRight && (
          <button
            ...
            className="text-2xl z-50 absolute right-2 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white px-7 py-4 cursor-pointer rounded-full shadow-md hover:bg-gray-700"
          >
            ❯
          </button>
        )}
